...
async function startQasimDev() {
    try {
        ...
        if (pairingCode && !isRegistered) {
            ...
            const { parsePhoneNumber: pn } = await import('awesome-phonenumber');
            if (!pn('+' + phoneNumberInput).valid) {
        ...
        QasimDev.ev.on('connection.update', async (s) => {
            ...
            if (connection == "open") {
                ...
                const { startAutoBio } = await import('./plugins/setbio.js');
...
// hot-reload removed for ESM compatibility
