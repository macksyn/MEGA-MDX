...
import {
  ...
  LOAN_TIERS, INTEREST_RATE, MIN_LOAN_AMOUNT, GRACE_PERIOD_MS, GARNISHMENT_RATE,
} from '../lib/loans.js';
...
async function sendLoanDetails(sock, message, chatId, channelInfo, loan) {
  ...
  await sock.sendMessage(chatId, {
    text:
      ...
      `Owed now: *${formatNumber(loan.balance)} coins* _(${(loan.interestRate * 100).toFixed(0)}%/week, compounding)_\n\n` +
  ...
  if (result.success) {
    ...
    await sock.sendMessage(chatId, {
      text:
        ...
        `Tier: ${l.tier}\n` +
        `Interest: ${(l.interestRate * 100).toFixed(0)}%/week, compounding\n` +
        `Due: ${l.termWeeks} week${l.termWeeks > 1 ? 's' : ''} from now\n\n` +
...
async function doRepay(sock, message, chatId, channelInfo, userId, amount) {
  ...
  if (!result.success) {
    const messages: Record<string, string> = {
      no_active_loan: `❌ You don't have an active loan.`,
...
async function runApplyAmountMenu(sock, message, chatId, userId, channelInfo, eligibility) {
  ...
  const result = await promptMenu(sock, message, chatId, userId, {
    ...
    subtitle: `${(INTEREST_RATE * 100).toFixed(0)}%/week compounding · ${eligibility.tier.termWeeks}w term`,
