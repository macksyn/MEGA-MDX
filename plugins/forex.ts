...
async function marketBoardText(): Promise<string> {
  ...
  let lines =
    ...
    `🏦 Bank  ${progressBar(pool, BANK_BAR_MAX)}  ${formatNumber(pool)} coins\n` +
    `${DIVIDER}\n`;
...
async function _handler(sock: any, message: any, args: string[], context: any) {
  ...
  if (args[0] === 'profile' || args[0] === 'stats') {
    ...
    return sock.sendMessage(chatId, {
      text:
        ...
        ` Total staked  ${formatNumber(profile.totalBet)} coins\n` +
        ` Total won     ${formatNumber(profile.totalWon)} coins\n` +
        ` Your RTP      ${rtpLine}`,
      ...channelInfo,
    }, { quoted: message });
  }
  ...
  // ── Deduct stake ───────────────────────────────────────────────
  const deducted = await deductCoins(userId, bet, { type: 'forex' });
  if (!deducted.success) {
    return sock.sendMessage(chatId, {
      text: `${header()}\n❌ You don't have enough coins for that trade.`,
      ...channelInfo,
    }, { quoted: message });
  }
  ...
  const orderLine = `${cfg.display}  ${dirLabel}   Bet: *${formatNumber(bet)}* coins\nEntry ≈ ${fmtPrice(entryPreview, cfg.decimals)}`;
  ...
  if (result.winAmount > 0) {
    await addCoins(userId, result.winAmount, { type: 'forex' });
  ...
  const finalText =
    ...
    `${eventLine}` +
    `${DIVIDER}\n` +
    `${cfg.display}  ${dirLabel}   Bet: *${formatNumber(bet)}* coins\n` +
