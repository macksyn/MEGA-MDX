...
async function saveBirthdayData(
  ...
  try {
    ...
    const doc: BirthdayDoc = {
      userId,
      name,
      // Prefer newly resolved phone; fall back to previously stored one
...
async function findByPhone(phone: string, sock?: any): Promise<BirthdayDoc | null> {
  try {
    const clean = phone.replace(/\D/g, '');
    if (!clean) return null;
    const birthdays = await getAllBirthdays();
    const entry = Object.values(birthdays).find(b => {
      // 1. Fastest path — stored phone field
      if (b.phone && b.phone.replace(/\D/g, '') === clean) return true;
      // 2. Match against phone JID
      if (!b.userId.includes('@lid') && b.userId.replace('@s.whatsapp.net', '').replace(/\D/g, '') === clean) return true;
      // 3. For LID userIds, resolve via store and compare
      if (b.userId.includes('@lid') && sock) {
        const lidNorm = b.userId.split('@')[0].split(':')[0];
        const resolved =
          sock.store?.contacts?.[b.userId]?.phone ||
          sock.store?.lidToPhone?.[lidNorm];
        if (resolved && resolved.replace(/\D/g, '') === clean) return true;
      }
      return false;
    });
    return entry || null;
  ...
  if (!busListenerRegistered) {
    ...
    bus.on('attendance:birthday', async (payload: any) => {
      try {
        ...
        // Resolve phone number so it is stored alongside the birthday record
        let phone: string | undefined;
        if (!userId.includes('@lid')) {
          phone = userId.split('@')[0].split(':')[0];
        } else {
          const raw = userId.split('@')[0].split(':')[0];
          phone =
            sock.store?.contacts?.[userId]?.phone ||
            sock.store?.lidToPhone?.[raw] ||
            undefined;
        ...
        const success = await saveBirthdayData(userId, name, birthdayData.displayDate, phone);
        if (success) {
          printLog('success', `[BIRTHDAY] 🎂 Auto-saved from attendance → ${name} (${birthdayData.displayDate})${phone ? ` [${phone}]` : ''}`);
        } else {
...
async function handleAll(sock: any, message: any, chatId: string, senderId: string): Promise<void> {
  if (!await checkAdminAccess(sock, chatId, senderId)) {
...
async function handleSetPhoto(sock: any, message: any, chatId: string, args: string[]): Promise<void> {
  ...
  const person = await findByPhone(phone, sock);
  if (!person) {
    return sock.sendMessage(chatId, {
...
async function resolveDisplayPhone(person: BirthdayDoc, sock: any): Promise<string> {
  // 1. Use the phone stored directly on the record — fastest path, no async needed
  if (person.phone) return person.phone;

  const userId = person.userId;
  const raw    = userId.split('@')[0].split(':')[0];
  if (!userId.includes('@lid')) return raw;

  // 2. Check store contacts
  const stored = sock.store?.contacts?.[userId]?.phone;
  if (stored) return stored;

  // 3. Check runtime lidToPhone map (populated from group events)
  const fromMap = sock.store?.lidToPhone?.[raw];
  if (fromMap) return fromMap;

  // 4. Ask Baileys signal repository directly — works without cached mapping
  try {
    const lidMapping = sock?.signalRepository?.lidMapping;
    const pnJid: string | null = lidMapping ? await lidMapping.getPNForLID(userId) : null;
    if (pnJid) {
...
// ── Shared admin access check ─────────────────────────────────────────────────

async function checkAdminAccess(sock: any, chatId: string, senderId: string): Promise<boolean> {
  if (await isOwnerOrSudo(senderId, sock, chatId)) return true;
  if (chatId.endsWith('@g.us')) {
    try { const r = await isAdmin(sock, chatId, senderId); return r.isSenderAdmin; } catch {}
  }
  return false;
}
...
async function handleListImages(sock: any, message: any, chatId: string): Promise<void> {
  ...
  if (withPhoto.length > 0) {
    ...
    for (const p of withPhoto) {
      const display = await resolveDisplayPhone(p, sock);
      msg += `• ${p.name} — ${p.birthday.displayDate} _(${display})_\n`;
    }
    msg += '\n';
  }
